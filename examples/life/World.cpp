...
void World::SetNext(Point2D point, bool value) {
  ...
  Point2D wrapPoint = WrapCoordinates(point);
  auto index = (wrapPoint.y * sideSize) + wrapPoint.x;
...
Point2D const World::WrapCoordinates(Point2D point) {
  int wrappedXValue = (point.x % sideSize + sideSize) % sideSize;
  int wrappedYValue = (point.y % sideSize + sideSize) % sideSize;

  return Point2D(wrappedXValue, wrappedYValue);
}
